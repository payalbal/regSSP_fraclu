...
filter.gbifcsv = function (gbif.downloaded.data,
                           ...
                           filter_fields = c("gbifid", "occurrenceid", "species",
                           ...
                           filter_basisofrecord = c("HUMAN_OBSERVATION",
                                                    "PRESERVED_SPECIMEN",
                                                    "OBSERVATION", 
                                                    "MATERIAL_SAMPLE",
                                                    "MACHINE_OBSERVATION"),
                           issue_geospatial = c("ZERO_COORDINATE",
                                                "COORDINATE_INVALID",
                                                ...
                                                "GEODETIC_DATUM_INVALID"),
                           issue_taxonomic = c("TAXON_MATCH_FUZZY", 
                           ...
                           return_data = TRUE)
{
  ...
  dat <- dat[!grep(paste0(c(issue_geospatial,issue_taxonomic, 
                            issue_basisofrecord, issue_date), 
                          collapse = "|"), dat$issue, perl = TRUE, value = FALSE)]
  ...
  dat <-
    dat[!which(sapply((strsplit(
      sub('0+$', '', as.character(dat$decimallongitude)), ".", fixed = TRUE
    )), function(x)
      nchar(x[2])) < 2)]
  dat <-
    dat[!which(sapply((strsplit(
      sub('0+$', '', as.character(dat$decimallatitude)), ".", fixed = TRUE
    )), function(x)
      nchar(x[2])) < 2)]
  ...
  dat <-
    dat[!grep(paste0(c("sp.", "spp", "sp"), "$", collapse = "|"),
              dat$species,
              perl = TRUE,
              value = FALSE)]
  ...
  gbif.nub.taxonomy <- gbif.nub.taxonomy[, .(taxonID, 
                                             canonicalName,
                                             taxonRank, 
                                             taxonomicStatus, 
                                             kingdom, phylum, 
  ...
  if(!is.null(subset.gbifnubtaxonomy.byclass)){
    ...
    check_list <-
      dat_species_list[which(!(
        dat_species_list %in% unique(gbif.nub.taxonomy$canonicalName)
      ), arr.ind = TRUE)]
  ...
  dat <- unique(dat, by =c("species", "decimallongitude", 
                           "decimallatitude"))
  ...
  if(is.null(select_fields)){
    select_fields = c("gbifid", "species", "decimallatitude", 
                      "decimallongitude", "taxonkey", "issue")
  } else {
  ...
  if(!is.null(output_folder))
  {
    ...
    writeLines("###",con = fileConn)
    writeLines(paste0("### Created ",Sys.time()," using the filter.gbifcsv() function."),con = fileConn)
    writeLines("###",con = fileConn)
    writeLines("################################",con = fileConn)
    writeLines("",con = fileConn)
    writeLines(paste0("Output data file = ", 
                      file.path(output_folder, output_name)),con = fileConn)
    writeLines(paste0("Domain mask applied = ", 
                      domain.mask@file@name),con = fileConn)
    writeLines(paste0("Data restricted to after ", 
                      start.year),con = fileConn)
    writeLines(paste0("Data restricted to spatial uncertainty < ",
                      spatial.uncertainty.m, "m"),con = fileConn)
    writeLines(paste0("Number of records before filtering = ",
                      n.rec.start),con = fileConn)
    writeLines(paste0("Number of records after filtering = ",
                      nrow(dat)),con = fileConn)
    writeLines("################################",con = fileConn)
    close(fileConn) 
    ## ***********************************************************************
  ...
  if(verbose)
  {
    ...
    msg2 = paste('These data have been also been written to ', 
                 file.path(output_folder, output_name))
    msg3 = paste("# records in raw data = ", n.rec.start)
    msg4 = paste("# records in filtered data = ", dim(dat)[1])
    msg5 = paste("# records removed (including spatial duplicates) =",
                 n.rec.start-dim(dat)[1])
