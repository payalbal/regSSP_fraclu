...
for (j in 1:length(countries)){
  ...
  for (i in 1:length(rcps)){
    ...
    if (country == 'vnm'){
      ...
      demand_2005 <- demand_2005[c(1,2,3,4,5)]
    } else if (country == 'aus') {
      urb_change <- (31346.1 - 21996.1)/21996.1 #Figures of urban population change from FAO
      demand_2005 <- demand_2005[c(1,2,3,4,5)]
    }
    an_urb_change <- urb_change/yrs
    demand_2050[,4] <- round(demand_2005[4] * (1 + c(1:(yrs)) * c(0, rep(an_urb_change, yrs-1))))
    ...
    demand_2050[,1] <- round(demand_2005[1] * (1 + colMeans(total_bysector[,2] * m_agr[-which(rownames(m_agr)%in%c("frs", "ctl")),]/100 * length(total_bysector[,2]))))
